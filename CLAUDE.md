...
- **iOS scale + safe-area (ADR-v4.1-005):** the whole calculator is fitted to the viewport with a CSS `transform: scale()` in `ScaledApp` (`main.tsx`). Safe-area insets are applied ONCE on the un-scaled outer frame via a **stylesheet** class `.scaled-app-frame` (`env()` in a React inline style is dropped by WKWebView — must be a stylesheet rule); `computeScale` fits into the inset-reduced area. `position: fixed` overlays (print bottom sheet) are `createPortal`-ed to `document.body` so the `transform` ancestor doesn't become their containing block. Scale recomputes on a `hp41:recompute-scale` event (overlay/PRGM/print toggles) + `visualViewport` + rAF/350ms (iOS keyboard-dismiss fires no reliable `resize`). Pinch-zoom locked (viewport meta + WebKit `gesture*` preventDefault). Vitest runs `globals:false` → portaled nodes need `afterEach(cleanup)`. Gotcha catalog: `reference_ios_gui_layout_gotchas` memory.
- **iOS ALPHA entry is keys-only (ADR-v4.1-003, supersedes Phase-55 TOUCH-04):** no iOS software keyboard. ALPHA-register AND "FUNCTION NAME?" entry use the on-screen HP-41 blue-letter keys; text shows on the main 14-seg display. On-screen ← and physical Backspace in ALPHA mode dispatch `alpha_backspace` (delete last char), NOT `alpha_clear`/`entry_backspace`. `AlphaTouchInput.tsx` is now dead code (cleanup pending). iOS `.key-touch-target` overlays are sized to the actual key (`keyPosition()`), not a fixed 44px dot, so the wide ENTER key (colSpan 2) is fully tappable.
- **Help `?` overlay = tabbed function index (ADR-v4.1-004, revises D-26.8):** two tabs — "Keyboard Shortcuts" (`key_path !== null`) and "All Functions" (every `status:"implemented"` entry across all 6 pools, grouped Module→Category). All-Functions rows are **tap-to-run** (`xeq_<display_name>` → execute, or insert-step in PRGM); 61 non-runnable ops (`NON_TAPPABLE`, pinned TS + Rust) render inert (D-07). Mnemonic fidelity: `CLREG`→`CLRG`, `CL SIGMA`→`CLΣ` (both keep back-compat aliases); `CLRALPHA` hidden via `OVERLAY_HIDDEN_ALIASES`. See `docs/hp41cv-divergences.md`.
- **Authentic single-step PRGM view (ADR-v4.1-006):** the GUI main display shows the current program step in PRGM mode (`from_state` `prgm_mode` branch → `format_step`, mirroring the CLI — restores D-25.6); the old multi-line listing (iOS sheet + desktop `.prgm-panel`) is removed. SST/BST navigate.
- **macOS menu-bar mode (ADR-v4.1-001):** On macOS only, the GUI runs as a status-item app — Accessory activation policy, no Dock icon, window starts hidden (`visible:false`), left-click tray toggles a borderless always-on-top popover, right-click menu = About / Start at Login / Quit. Applied at runtime under `#[cfg(target_os = "macos")]` in `tray.rs` (NOT in `tauri.conf.json`, so Windows/Linux keep a normal decorated window). Pure logic in `tray_helpers.rs` (geometry + flicker-guard) is unit-tested. Auto-hide-on-blur is suppressed around native file dialogs via `PopoverState.suppress_hide` (RAII guard in `commands.rs`); in-app modals don't trigger window blur. `HP41_SHOW_ON_START=1` forces a normal window (E2E). Autostart via `tauri-plugin-autostart`; GUI Cargo features gained `tray-icon` + `image-png`. The mode is **user-selectable** via `GuiPrefs.macos_launch_mode` (`"menu-bar"` default | `"window"`); the Settings panel exposes a macOS-only radio (gated by the `is_macos` command) that persists via `set_pref` and offers an immediate `restart_app`. Precedence: `HP41_SHOW_ON_START` > `"window"` pref > menu-bar. `PopoverState.menu_bar_active` gates blur-auto-hide so window mode stays visible; existing saves stay menu-bar via `#[serde(default)]`.
- **Single-instance + global hotkey (ADR-v4.3-001/002):** A second launch never creates a duplicate tray icon — `tauri-plugin-single-instance` (desktop-gated dep, registered **first** in the builder per plugin docs) routes the relaunch to the running instance, which surfaces itself (`focus_existing_instance` → `tray::surface_popover` on macOS menu-bar, else show+focus). A **macOS-only, configurable** global hotkey (default `⌃⌥⌘H`, `prefs::DEFAULT_GLOBAL_SHORTCUT` = single source of truth) toggles the popover via `tauri-plugin-global-shortcut` (`cfg(target_os="macos")` dep; Carbon `RegisterEventHotKey` → no Accessibility permission). All three triggers (tray click, hotkey, relaunch) funnel through one `tray::toggle_popover`/`surface_popover`; `PopoverState.last_tray_rect` (captured on tray click) positions the popover, with `compute_fallback_position` (unit-tested) as the pre-first-click top-right anchor. Accelerator stored in `GuiPrefs.global_shortcut` (`#[serde(default)]`); the **Rust side validates** — `set_pref` parses + live-re-registers and returns `Err` on an unparseable value, so the `ShortcutRecorder.tsx` overlay (macOS-gated, portaled, capture-phase keydown) only reflects an accepted hotkey. **No new Tauri command/capability** (Rust-side registration; recorder reuses `set_pref`). `prefs.rs` stays platform-neutral (only repairs an *empty* accelerator — `Shortcut` parsing is macOS-only); iOS build unaffected (verify with `cargo check --target aarch64-apple-ios`).
