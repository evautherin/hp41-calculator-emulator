...
//
//! Phase 42 Plan 01 / D-42.1 — Unified meta-test: each `Op` variant registered
//! in `math1_resolve`, `stat1_resolve`, and `time_resolve` must have at least 5
//! test functions mentioning it (Pitfall 16 guard).
//!
//! **Replaces (D-42.14):**
...
//! (external-only tests). For Stat 1 and Time, both tokens are checked because
//! inline tests call internal functions directly.
//!
...
//! - Math 1 = 45, Stat 1 = 26, Time = 35 (total 106).
...
/// Core PascalCase → `op_<snake_case>` converter (shared by stat1 and time).
...
/// Meta-test (Phase 42 / D-42.1 / T-42-01): every Op variant registered in any
/// of the three XROM module resolvers (math1_resolve, stat1_resolve,
/// time_resolve) must have ≥ 5 test function references.
///
/// Scans per module:
...
fn each_xrom_op_has_at_least_5_tests() {
    ...
    assert!(
        ...
        "Pitfall 16 violation — XROM module variants with insufficient test coverage \
         across all 3 modules (Math 1 + Stat 1 + Time):\n{}",
        failures.join("\n")
    );
}
