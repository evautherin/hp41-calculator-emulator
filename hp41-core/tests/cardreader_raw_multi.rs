...
fn decode_all_single_program_returns_vec_of_one() {
    ...
    assert_eq!(programs.len(), 1, "single-program stream must yield Vec of length 1");
    assert_eq!(programs[0].ops, vec![Op::Add, Op::Sub], "ops must match encoded ops");
    assert_eq!(
        programs[0].byte_len,
        bytes.len(),
...
fn decode_all_two_programs_yields_correct_segments() {
    ...
    assert_eq!(programs.len(), 2, "two concatenated programs must yield Vec of length 2");
...
fn decode_all_three_programs_yields_vec_of_three() {
    ...
    let programs = decode_all_programs(&combined).unwrap();
    assert_eq!(programs.len(), 3, "three concatenated programs must yield Vec of length 3");
    assert_eq!(programs[0].ops, vec![Op::Add]);
...
fn xrom_synthetic_byte_round_trip_through_decode_all() {
    ...
    let original_ops = vec![Op::Lbl("XTEST".to_string()), Op::SyntheticByte(0xAA), Op::Rtn];
