...
fn matrix_workflow_is_neutral_lift() {
    ...
    assert!(
        ...
        "MatrixWorkflow must not modify lift_enabled"
    );
}
...
fn matrix_workflow_does_not_write_print_buffer() {
    ...
    assert!(
        ...
        "MatrixWorkflow must not write to print_buffer"
    );
}
...
fn mat_size_enables_lift() {
    ...
    assert!(
        state.stack.lift_enabled,
        "MatSize must set lift_enabled = true"
    );
}
...
fn mat_size_returns_zero_when_r14_is_zero() {
    ...
    assert_eq!(
        state.stack.x,
        HpNum::zero(),
        "MatSize returns R14 verbatim (0)"
    );
}
...
fn mat_vmat_writes_to_print_buffer() {
    ...
    assert!(
        !state.print_buffer.is_empty(),
        "MatVmat must write to print_buffer"
    );
}
...
fn mat_vmat_produces_n_squared_lines() {
    ...
    assert_eq!(
        ...
        9,
        "MatVmat on 3×3 must produce 9 lines"
    );
}
...
fn mat_vmat_is_neutral_lift() {
    ...
    assert!(
        !state.stack.lift_enabled,
        "MatVmat must not modify lift_enabled"
    );
}
...
fn mat_edit_is_neutral_lift() {
    ...
    assert!(
        state.stack.lift_enabled,
        "MatEdit must not modify lift_enabled"
    );
}
...
fn mat_edit_without_matrix_returns_invalid_op() {
    ...
    assert!(
        result.is_err(),
        "MatEdit without active matrix must return error"
    );
}
...
fn mat_det_enables_lift() {
    ...
    assert!(
        state.stack.lift_enabled,
        "MatDet must set lift_enabled = true"
    );
}
...
fn mat_det_singular_returns_near_zero() {
    ...
    assert!(
        det.abs() < 1e-7,
        "det of singular matrix must be near zero, got {det}"
    );
}
...
fn mat_det_order_15_returns_error() {
    ...
    assert!(
        result.is_err(),
        "MatDet with ORDER=15 must return error (cap is 14)"
    );
}
...
fn mat_inv_is_neutral_lift() {
    ...
    assert!(
        !state.stack.lift_enabled,
        "MatInv must not modify lift_enabled"
    );
}
...
fn mat_inv_well_conditioned_no_error() {
    ...
    assert!(
        result.is_ok(),
        "MatInv on well-conditioned matrix must return Ok"
    );
    assert!(
        state.modal_prompt.is_none(),
        "No NO SOLUTION for well-conditioned matrix"
    );
}
...
fn mat_simeq_is_neutral_lift() {
    ...
    assert!(
        !state.stack.lift_enabled,
        "MatSimeq must not modify lift_enabled"
    );
}
...
fn mat_vcol_writes_to_print_buffer() {
    ...
    assert_eq!(
        state.print_buffer.len(),
        2,
        "MatVcol on 2×2 must produce 2 lines (B1, B2)"
    );
}
...
fn mat_vcol_lines_start_with_b_prefix() {
    ...
    assert!(
        state.print_buffer[1].starts_with("B2="),
...
fn mat_vcol_is_neutral_lift() {
    ...
    assert!(
        !state.stack.lift_enabled,
        "MatVcol must not modify lift_enabled"
    );
}
...
fn mat_vcol_without_matrix_returns_error() {
    ...
    assert!(
        result.is_err(),
        "MatVcol without active matrix must return error"
    );
}
