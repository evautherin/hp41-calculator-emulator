...
fn loads_synthetic_v22_save_without_v3_fields() {
    ...
    // Phase 28 + Phase 38 + Phase 43 fields must be at their documented defaults.
    // NOTE: Phase 43 (v3.3) flipped `default_xrom_modules()` to `0b0001_1111`
    // (Math 1 + Stat 1 + Time Module + Advantage Pac pre-loaded) per ADV-FW-01.
    // A v2.2 save file containing NO `xrom_modules` field therefore now
    // deserializes with the v3.3 default; older save files that explicitly
    // persist lower values are upgraded by `CalcState::migrate_after_load()`.
    assert_eq!(
        state.xrom_modules, 0b0001_1111,
        "v2.2 save must produce v3.3 default xrom_modules = 0b0001_1111 (Math 1 + Stat 1 + Time + Advantage Pac pre-loaded)"
    );
    assert!(
        !state.complex_mode,
