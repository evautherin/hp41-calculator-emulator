...
- [ ] **BUILD-01**: Build approach (Tauri v2 Mobile vs. native SwiftUI + Rust FFI) is decided via a hands-on `cargo tauri ios build` spike and captured in ADR `v4.1-001-build-approach.md`
- [ ] **BUILD-02**: `hp41-core` compiles to the iOS targets and the app builds and launches in the iOS Simulator (one key dispatches through the existing engine)
- [ ] **BUILD-03**: The app builds, installs, and runs on a physical iPhone
- [ ] **BUILD-04**: Bundle ID `ch.talent-factory.hp41` is registered in App Store Connect (App ID + app record)
...
- [ ] **TOUCH-02**: Portrait-locked layout respects safe-area insets (notch / Dynamic Island / home indicator)
...
- [ ] **TOUCH-05**: Key presses produce haptic feedback
...
| TOUCH-02 | Phase 55 | Pending |
| TOUCH-03 | Phase 55 | Pending |
| TOUCH-04 | Phase 55 | Pending |
| TOUCH-05 | Phase 55 | Pending |
