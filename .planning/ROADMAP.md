...
**Plans**: 1 plan
- [x] 56-01-PLAN.md — `tauri.ios.conf.json` (backgroundThrottling: throttle, complete window object) + needsTickRef + gated `visible` branch firing one `tick_time` on foreground return + on-device checkpoint + touch R/S stopwatch parity fix (LIFE-01, LIFE-02)
**UI hint**: yes
...
| 57. Signing + TestFlight Pipeline | 2/4 | In Progress|  |
