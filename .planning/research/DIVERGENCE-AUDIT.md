...
*All three UNC items were verified against the HP-41C Operating Manual (HP 00041-90259, June 1980) in Phase 66. Dispositions are now final.*

| ID | Name | OM Citation | Disposition | Resolution |
|----|------|-------------|-------------|------------|
| UNC-01 | Back-arrow clears error message | HP-41C OM p.15: "Pressing [←] also clears error messages from the display." | **Already-correct** — `hp41-cli/src/app.rs` line 974–975 calls `backspace_entry()` then sets `self.message = None`, exactly matching OM behavior. No code change required. | Verified Phase 66; no fix needed. |
| UNC-02 | Flags 21/55 gate PRX/PRA/PRSTK printing | HP-41C OM p.53: Flag 21 = "Printer Enable"; p.54: Flag 55 = "Printer Existence"; p.57-58: print functions error with NONEXISTENT when printer not connected. | **Divergent — fixed Phase 66.** PRX/PRA/PRSTK now gate on `flag_get(state.flags, 55) \|\| flag_get(state.flags, 21)` and return `HpError::NonExistent` when neither is set. `HpError::NonExistent` variant added to `hp41-core/src/error.rs`. Print tests reworked to set flag 55 in setup; one new test (`test_prx_returns_nonexistent_when_no_printer_flag`) added. Note: flag 25 = "Error Ignore" (generic, NOT a printer flag) — fix uses flags 21/55 only. | Fixed Phase 66 (Plan 66-02). |
| UNC-03 | SIZE reduction shows MEMORY LOST display | HP-41C OM p.19: SIZE reduction silently truncates highest-numbered registers — no special display message described. OM p.57: "MEMORY LOST — The Continuous Memory of the calculator has been cleared" (power/hardware event only, not a SIZE event). | **Already-correct** — SIZE reduction does NOT show MEMORY LOST on real hardware. The `op_size` comment claiming "hardware-faithful 'MEM LOST'" was misleading and was corrected in Phase 66 (Plan 66-02) to read: "hardware-faithful: data truncated silently, no MEMORY LOST display (OM p.19)". No logic change; comment-only fix. | Verified Phase 66; comment corrected in Plan 66-02. |
...
All gaps and uncertain behaviors addressed in the v4.3 Hardware Fidelity milestone, with OM citations and closing phases.
...
*Deferred items (FGAP-06, FGAP-09): out of scope for v4.3; see individual rows above.*

---

*Last updated: 2026-06-10 — v4.3 Closure Ledger added; FGAP/UNC resolution marks updated (Phase 66).*
