...
#### CLAUDE.md `### v3.1 additions` block — fix-up of missing v3.0 precedent

- **D-35.5: CLAUDE.md `### v3.0 additions` block does NOT currently exist; Phase 35 adds ONLY the `### v3.1 additions` block (no v3.0 back-fill).** Pattern-mapping (Phase 35 plan-phase, 2026-05-23) discovered that earlier references in this CONTEXT.md to "CLAUDE.md `### v3.0 additions` block (lines 117–168 region)" are factually wrong — that block lives in `docs/architecture-history.md` at line 117, NOT in CLAUDE.md. CLAUDE.md was restructured at v3.0 ship-time into `## Frozen Invariants` / `## Tech Stack` / `## Quality Gates` / `## Key Files` sections (current shape) and the additions-block scaffolding planned in Phase 30 D-30.8 was never landed. Phase 35 honors ROADMAP Success Criterion #4 literally by adding **only** the `### v3.1 additions` block (user-confirmed Phase 35 plan-phase 2026-05-23).
  - **Insertion point (REVISED):** after the `## Frozen Invariants` section and **before** `## Tech Stack` (around current CLAUDE.md line 119, immediately after the Free42 GPL-contamination guard sub-section). Planner picks the exact line; the substance is fixed.
  - **Content shape:** sub-section per phase mirroring CONTEXT.md `<specifics>` D-35.3 headings (Phase 33 / 34 / 35 fully populated; Phase 36 / 37 stub-headers with `(in progress)` markers).
  - **Why Option B (v3.1-only):** ROADMAP Success Criterion #4 demands a CLAUDE.md `### v3.1 additions` block but says nothing about v3.0. Back-filling v3.0 (Option A) is busywork for an already-shipped milestone whose narrative is preserved in `architecture-history.md`. Skipping the block entirely (Option C) violates SC #4 literally and would require a verification waiver. Option B keeps the delta tight, satisfies SC #4 verbatim, and follows the incremental-population intent of D-30.8 (which simply was never landed last time). When Phase 35 ships, future v3.2 milestones can add a `### v3.2 additions` block as a sibling — by then v3.1 will be the precedent.
  - **Supersedes:** all prior CONTEXT.md references to "CLAUDE.md `### v3.0 additions` block (lines 117–168 region)" as structural template — those should be read as "see `docs/architecture-history.md` §`v3.0 additions` (line 117 region) for the narrative template; the CLAUDE.md `### v3.1 additions` block is introduced fresh in Phase 35 (no v3.0 sibling exists or is created)".
...
- **`### v3.1 additions` block insertion point in CLAUDE.md:** see D-35.5 — the assumed `### v3.0 additions` block does not exist in current CLAUDE.md. Insertion point is after `## Frozen Invariants` and before `## Tech Stack` (current line 119 region, immediately after the Free42 GPL-contamination guard sub-section). Planner picks the exact line; structure mirrors `docs/architecture-history.md` §`v3.0 additions` (one sub-section per phase + tail note about preserved invariants).
...
- `CLAUDE.md` (repo root) — Frozen Invariants section (Core engine math1/ freeze amendment target per ADR-v3.1-004); Tech Stack section; Key Files table; JSON canonical data flow lines 122–129. NOTE per D-35.5: CLAUDE.md does NOT currently have a `### v3.0 additions` block — Phase 35 inserts the FIRST `### v3.x additions` block (v3.1 only) immediately after `## Frozen Invariants` and before `## Tech Stack` (current line 119 region).
...
- CLAUDE.md `### v3.0 additions` block (lines 117–168 region) — **DOES NOT EXIST** per D-35.5; the structural template for the new `### v3.1 additions` block in CLAUDE.md is the architecture-history.md `## v3.0 additions` section (line 117 region of that file) — same shape, sub-section per phase. Phase 35 Plan 35-04 introduces the first `### v3.x additions` block in CLAUDE.md (v3.1 only — no v3.0 back-fill).
...
- **`docs/architecture-history.md` `## v3.0 additions (Math Pac I Emulation, Phases 28–32)` section (line 117 region of that file)** — structural template for the new `### v3.1 additions (Stat 1 Pac Emulation, Phases 33–35 — 36–37 IN PROGRESS)` block in CLAUDE.md per D-35.5; sub-section-per-phase shape mirrors directly. NOTE: the assumed CLAUDE.md `### v3.0 additions` block does not exist — Phase 35 introduces the first `### v3.x additions` block in CLAUDE.md.
...
- **`CLAUDE.md` insertion points (Plan 35-04):**
  - `### v3.1 additions` block (per D-35.5): introduce the first `### v3.x additions` block in CLAUDE.md — insertion point is after `## Frozen Invariants` and before `## Tech Stack` (current CLAUDE.md line 119 region, immediately after the Free42 GPL-contamination guard sub-section). No v3.0 back-fill.
